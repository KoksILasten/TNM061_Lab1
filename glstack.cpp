...
int main() {
    ...
    neptuneTexture.createTexture("textures/2k_neptune.tga");

    Texture venusTexture;
    venusTexture.createTexture("textures/2k_venus_surface.tga");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        modelViewStack.rotY(0.2f * time);             // Venus orbit rotation
        modelViewStack.translate(-1.0f, 0.0f, 0.0f);  // Venus orbit radius
        modelViewStack.push();                        // Save the matrix before the Venus's rotation

        modelViewStack.rotY(5.0f * time);  // Venus's rotation around its axis
        modelViewStack.scale(0.1f); // Scale unit sphere to radius 0.1
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the Venus
        ...
        modelViewStack.rotY(0.2f * time);            // Neptune orbit rotation
        modelViewStack.translate(0.5f, 1.0f, 0.6f);  // Neptune orbit radius
        modelViewStack.push();                       // Save the matrix before the Neptune's rotation

        modelViewStack.rotY(5.0f * time);  // Neptune's rotation around its axis
        modelViewStack.rotX(static_cast<float>(-M_PI_2/2));  // Orient the poles along Y axis instead of Z
        modelViewStack.scale(0.3f);        // Scale unit sphere to radius 0.1
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the Neptune
        glBindTexture(GL_TEXTURE_2D, earthTexture.texID);
        earthSphere.render();
        ...
        modelViewStack.rotY(5.0f * time);                 // Earth's rotation around its axis
        modelViewStack.rotX(static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        modelViewStack.rotX(0.409105f);  // Tilting the Earth's axis by 23.5 degrees
		...
		// Moon
        modelViewStack.rotY(0.5 * time);
        modelViewStack.translate(0.4f, 0.0f, 0.0f);
        modelViewStack.push();
		
        modelViewStack.rotY(-M_PI_2/5.0f * time);  // Moon orbit rotation
        modelViewStack.scale(0.1); // Moon scale
